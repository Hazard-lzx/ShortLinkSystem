...
/**
 * Feign 降级：跳转服务不可用时不阻塞管理端主业务，
 * 缓存一致性由缓存过期时间兜底（最终一致）
 */
@Slf4j
@Component
public class ShortLinkApiFeignFallback implements ShortLinkApiFeign {

    @Override
    public Result<Void> addBloomFilter(String shortCode) {
        log.warn("调用跳转服务[布隆过滤器写入]降级：shortCode={}，新短链首次访问将回源查库重建缓存", shortCode);
        return Result.fail("布隆过滤器写入降级");
    }

    @Override
    public Result<Void> evictCache(String shortCode) {
        log.warn("调用跳转服务[缓存清理]降级：shortCode={}，等待缓存自然过期实现最终一致", shortCode);
